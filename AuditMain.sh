...
#folder storing code files
storageLocationUSB=/Volumes/*
storageLocationPC=/Users/user/Documents/Git/PowerON/PowerOn/scripts/MacAuditScripts
sourceFolder=/Audit
scriptFileName=/utilities.sh

#calling script
source $storageLocationPC$sourceFolder$scriptFileName
connect_to_wifi
...
elif [[ $USB == *"USB 3.0 Bus"* ]]; then
	echo -e "\tUSB 3.0 (Type A port)"
elif [[ $USB == *"USB 2.0 Bus"* ]]; then
	echo -e "\tUSB 2.0 (Type A port)"
fi


if [[ $ThunderSpeed == *"40"* ]]; then
...
elif [[ $ThunderSpeed == *"20"* ]]; then
	echo -e "\tThunderbolt 2"
elif [[ $ThunderSpeed == *"10"* ]]; then
...
if [[ $BattInstalled == *"Yes"* ]]; then
  ...
  if [[ "$IS_LAPTOP" == "" ]]; then
      BATTERY_HEALTH=null
  elif [ $BATTERY_HEALTH -gt 100 ];then
      BATTERY_HEALTH=100
  fi


#check if Apple Silicon battery health or intel battery health
#print in green if healthy
#print in red if not healthy
#print in yellow if battery is above 80% and below 70% and from Apple (gray zone)

  if [[ $M1 == "M" ]]; then

    CYCLE_COUNT=$(echo "$MBATT" | grep -E "Cycle Count" | awk '{print $3}')
    BATTERY_HEALTH_APPLE_SILLICON=$(echo "$MBATT" | grep -E "Maximum Capacity" | awk '{print $3}' | sed 's/%//')

    #testing
    #BATTERY_HEALTH_APPLE_SILLICON=80
    #MBATT="Cycle Count: 67 \n Condition: Service Required \n Maximum Capacity: 97%"
    #CYCLE_COUNT=1000
    #echo CYCLE_COUNT: $CYCLE_COUNT

    if echo "$MBATT" | grep -q "Service Required"; then
      echo -e "${RED}$MBATT${RESET}"
      printf '\033[1m'
    elif [ "$BATTERY_HEALTH_APPLE_SILLICON" -lt 70 ] || [ "$CYCLE_COUNT" -gt 999 ]; then
      echo -e "${RED}$MBATT${RESET}"
    elif [ "$BATTERY_HEALTH_APPLE_SILLICON" -lt 80 ]; then
      echo -e "${YELLOW}$MBATT${RESET}"
      echo -e "${BOLD}${YELLOW}Note: Apple units pass at 70% battery health, 80% for PowerON.${RESET}${RESET}"
    else
      echo -e "${GREEN}$MBATT${RESET}"
    fi

  else

    INTEL_CYCLE_COUNT=$(echo "$SYS" | grep -E "Cycle Count" | awk '{print $3}')

    if echo "$SYS" | grep -q "Service Recommended" || [ "$INTEL_CYCLE_COUNT" -gt 999 ]; then
